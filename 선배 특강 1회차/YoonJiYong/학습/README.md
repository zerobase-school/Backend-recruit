...
첫 번째 슬럼프는, 자료구조 중 트리구조를 학습하면서 겪었습니다.
두 번째 슬럼프는, 공공 API를 활용한 Wifi 리스트 가져오기 프로젝트를 할 때였습니다.
...
- **협업 프로젝트 활용법**
  1. 반드시 완성도를 높일 것
     - 완성도를 높이기 위한 과정이 현업에서 가장 중요하게 생각하는 역량이 밀집해있는 과정
       1. 서비스 로직에 대한 고민
       2. 리펙토링을 통한 코드, 메서드 정리
       2. 예외 처리 및 Validation에 대한 고민
       3. 쿼리 최적화
       4. 프로젝트의 아키텍쳐 및 패키지 구조
  2. 서버 배포 과정까지 진행해볼 것!
...
> 정리 팁
> - 핵심: `얼마나 어려운 문제를 해결했느냐` 보다 `어떻게 문제를 정의하고 어떤 생각의 흐름으로 문제에 접근하고 해결했는가` 가 더 중요.
> - 트러블슈팅 탬플릿을 만들어서 작성할 것.
>   1. 문제 상황: 어떤 동작을 했을 때 어떤 문제가 발생했는가. (문제 상황을 재현하는 것 자체가 쉽지 않는 경우도 있어, 문제가 발생하면 캡쳐하는 것을 습관화 하면 좋음)
>   2. 원인: 문제의 핵심 원인을 파악하기.
